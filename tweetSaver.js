function tweetLookUp() {
  ...
  if (tweetQuery !== "") {
    $.ajax({
      url:
        "http://tweetsaver.herokuapp.com/?q=" +
        tweetQuery +
        "&count=" +
        countQuery,
      type: "GET",
      ...
      success: function (response) {
        ...
        for (var i = 0; i < tweetArray.length; i++) {
          ...
          if (typeof response.tweets.retweetedStatus != "undefined") {
            ...
            tweetProfileImage =
              response.tweets[i].retweetedStatus.user.profileImageUrlHttps;
          }

          feedHTML +=
            '<div class="twitter-item"><div class="tweet-image" width="20px"><span><a href="https://twitter.com/' +
            tweetHandle +
            '" target="_blank"><img src="' +
            tweetProfileImage +
            '" width="42" height="42" alt="twitter icon" /></a></span><span class="tweetprofilelink"><strong><a href="https://twitter.com/' +
            tweetHandle +
            '" target="_blank">' +
            tweetUser +
            '</a></strong><br /> <a href="https://twitter.com/' +
            tweetHandle +
            '" target="_blank">@' +
            tweetHandle +
            '</a></span></div><div class="twitter-text"><p>' +
            tweetText +
            "</p></div></div>";
        }
